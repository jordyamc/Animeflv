...
public class SSDPDevice {
    ...
    public SSDPDevice(String url, String ST) throws Exception {
        this(new URL(url), ST);
    }

    public SSDPDevice(URL urlObject, String ST) throws Exception {
        if (urlObject.getPort() == -1) {
            baseURL = String.format("%s://%s", urlObject.getProtocol(), urlObject.getHost());
        } else {
            baseURL = String.format("%s://%s:%d", urlObject.getProtocol(), urlObject.getHost(), urlObject
                    .getPort());
        }
    ...
    public void parse(URL url) throws Exception {
        ...
        if (applicationURL != null && !applicationURL.substring(applicationURL.length() - 1)
                .equals("/")) {
