...
public class WebOSTVServiceSocketClient extends WebSocketClient implements ServiceCommandProcessor {
    ...
    protected void handleMessage(JSONObject message) {
        ...
        if ("response".equals(type)) {
            ...
            if (!(mService.getServiceConfig() instanceof WebOSTVServiceConfig)) {
                mService.setServiceConfig(new WebOSTVServiceConfig(mService.getServiceConfig()
                        .getServiceUUID()));
            }
            ...
            if (payload instanceof JSONObject) {
                ...
                ((WebOSTVServiceConfig) mService.getServiceConfig()).setServerCertificate(customTrustManager
                        .getLastCheckedCertificate());
            ...
            if (mService.getServiceConfig().getServiceUUID() != null) {
                if (!mService.getServiceConfig()
                        .getServiceUUID()
                        .equals(jsonObj.optString("deviceUUID"))) {
    ...
    private void helloTV() {
        ...
        @SuppressWarnings("deprecation") int width = display.getWidth(); // deprecated, but still needed for supporting API levels 10-12

        @SuppressWarnings("deprecation") int height = display.getHeight(); // deprecated, but still needed for supporting API levels 10-12
    ...
    protected void setupSSL() {
        try {
            ...
            customTrustManager.setExpectedCertificate(((WebOSTVServiceConfig) mService.getServiceConfig())
