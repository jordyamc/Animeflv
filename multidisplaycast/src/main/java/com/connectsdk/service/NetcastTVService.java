...
public class NetcastTVService extends DeviceService implements Launcher, MediaControl, MediaPlayer, TVControl, VolumeControl, ExternalInputControl, MouseControl, TextInputControl, PowerControl, KeyControl {
    ...
    public void connect() {
        ...
        if (DiscoveryManager.getInstance().getPairingLevel() == PairingLevel.ON) {
            if (((NetcastTVServiceConfig) serviceConfig).getPairingKey() != null && ((NetcastTVServiceConfig) serviceConfig)
                    .getPairingKey()
                    .length() != 0) {
            ...
            Util.runInBackground(new Runnable() {
                ...
                @Override
                public void run() {
                    httpServer = new NetcastHttpServer(NetcastTVService.this, getServiceDescription()
                            .getPort(), mTextChangedListener);
    ...
    public void getAppState(final LaunchSession launchSession, final AppStateListener listener) {
        String requestURL = String.format(Locale.US, "%s%s", getUDAPRequestURL(UDAP_PATH_APPTOAPP_DATA), String
                .format(Locale.US, "/%s/status", launchSession.getAppId()));
            ...
            public void onSuccess(List<ChannelInfo> channelList) {
                ...
                for (int i = 0; i < channelList.size(); i++) {
                    ...
                    try {
                        ...
                        if (Integer.valueOf(major) == majorNumber && Integer.valueOf(minor) == minorNumber) {
