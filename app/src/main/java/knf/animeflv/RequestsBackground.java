...
class RequestsBackground extends AsyncTask<String, String, Void> {
    private Context context;
    private TaskType taskType;

    RequestsBackground(Context cont, TaskType task) {
        context = cont;
        this.taskType = task;
    }
    ...
    protected Void doInBackground(String... params) {
        if (NetworkUtils.isNetworkAvailable()) {
            ...
            if (taskType == TaskType.NOT) {
                url = new Parser().getInicioUrl(TaskType.NORMAL, context) + "?certificate=" + new Parser().getCertificateSHA1Fingerprint(context);
            }
            ...
            try {
                ...
                client.get(url, null, new TextHttpResponseHandler() {
                    ...
                    public void onFailure(int statusCode, Header[] headers, String responseString, Throwable throwable) {
                        Log.e("Error RequestBackground", responseString);
                        makeDesicion("");
                    }
    ...
    private void makeDesicion(String s) {
        if (taskType == TaskType.NOT && new Parser().checkStatus(s.trim()) == 0) {
            ...
            if (NetworkUtils.isNetworkAvailable() && !s.trim().equals("")) {
                if (!file.exists()) {
                    ...
                    if (FileUtil.isJSONValid(txt) && FileUtil.isJSONValid(s)) {
    ...
    private void DescargarActualizacion(final File descarga, final String s) {
    ...
    private void Descargar(String aid, String num, String titulo, String eid) {
        int not = Integer.parseInt(PreferenceManager.getDefaultSharedPreferences(context).getString("sonido", "0"));
        Uri ring = UtilSound.getSoundUri(not);
        NotificationCompat.Builder mBuilder =
                new NotificationCompat.Builder(context)
