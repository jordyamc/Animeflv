...
public class ExternalStream {
    ...
    public void Stream(String eid, String url) {
        try {
            ...
            context.startActivity(new Intent(Intent.ACTION_VIEW, Uri.parse(url)).addFlags(Intent.FLAG_ACTIVITY_NEW_TASK).setDataAndType(Uri.parse(url), "video/mp4").putExtra("title", DirectoryHelper.get(context).getTitle(aid) + " " + numero));
