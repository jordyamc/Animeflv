...
public class UpgradeService extends IntentService {
    private static final int DOWNLOAD_NOTIFICATION_ID = 59348;
    private static final int FINISH_NOTIFICATION_ID = 58877;
    private NotificationManagerCompat manager;
    private NotificationCompat.Builder downloading;
    private File upgradeFile;

    public UpgradeService() {
        super("Animeflv Upgrade Service");
    }
    ...
    protected void onHandleIntent(@Nullable Intent intent) {
        onPreDownload();
        upgradeFile = Keys.Dirs.getUpgradeFile();
        if (upgradeFile.exists()) upgradeFile.delete();
        final ThinDownloadManager downloadManager = new ThinDownloadManager();
        final DownloadRequest downloadRequest = new DownloadRequest(Uri.parse(Keys.Url.UPGRADE))
                .setDestinationURI(Uri.fromFile(upgradeFile))
    ...
    private void onSetProgress(int progress) {
        NotificationCompat.Builder builder = getDownloadingBuilder()
                ...
                .setContentTitle("Descargando aplicacion...");
        getManager().notify(DOWNLOAD_NOTIFICATION_ID, builder.build());
    }
    ...
    private void onFinishDownload(boolean success) {
        ...
        downloading = null;
        NotificationCompat.Builder builder = getDownloadingBuilder()
                .setContentTitle("UKIKU - Instalacion")
        ...
        if (success) {
            ...
            Intent intent = new Intent(Intent.ACTION_INSTALL_PACKAGE, FileUtil.init(this).getUriForFile(upgradeFile));
